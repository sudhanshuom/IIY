...
<div class="wrapper">
    <div class="inner">
     ...
     <br>
      <div class="container">
        	<div align="center" style="color: #ff5722; font-size:18px">Student Attendance Status
...
<?php echo date("d-m-Y",strtotime($today)); ?>
	...
	$sql="Select * from stu_attendence WHERE DATE='$today'";
				foreach ($dbh->query($sql) as $row)
					{
						//$att=$row['ATT'];	
						if($studentid=="")$studentid=$row['STUID']; else $studentid.=','.$row['STUID'];	
						$stts=$row['STTS'];	
...
if($studentid!="")
{
	$sql="Select * from stu_info WHERE ID IN(".$studentid.") order by ID ASC";
				foreach ($dbh->query($sql) as $row)
					{
					if($att[$x]==2 || $att[$x]==1){
...
<i class="fa fa-phone"></i> <a href="tel:<?php echo $row['MB']; ?>"><?php echo $row['MB']; ?></a></h6>
  ...
  <?php
	...
	echo 'No Attendence Marked Today';
	}
  ?>       
