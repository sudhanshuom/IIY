     ...
     <br>
      <div class="container">
       ...
       <div class="row">
		...
		$subnm=findnm("stu_sub","SUB"," WHERE ID=$subid",$dbh);
        ...
        <div class="col-xs-12"><br>
               ...
               <p style="text-align:center; font-weight:bold;color: #ff5722;">Subject</p>
              	<select name="subid" class="form-control sub">
                    	<?php
						createoptions("stu_sub WHERE ID in(".$classids.")  ","ID","SUB",$subid,$dbh)
...
<form method="POST" action="save_dm.php" enctype="multipart/form-data">
     <br>
<?php
$dt=date("Y-m-d");
...
?>
              <p style="text-align:center; font-weight:bold;color: #ff5722;">Maximum Marks</p>
       <input type="test" id="tot" class="form-control" name="tot" value="<?php echo $tot; ?>"/>
...
{		
 $sql = "Select * from stu_info WHERE CLS='$classid'";
	...
	$mfound=findnm("stu_dmarks","ID"," WHERE DATE='".$dt."' AND SID=".$row['ID']." AND SUBID=".$subid."",$dbh);
	$omarks=findnm("stu_dmarks","OBM"," WHERE DATE='".$dt."' AND SID=".$row['ID']." AND SUBID=".$subid."",$dbh);

	if($mfound=="")	$dbh->exec("INSERT INTO stu_dmarks(DATE,SID,SUBID,AID) VALUES ('$dt','".$row['ID']."','$subid','$stuid')");
				...
				echo '&nbsp; Class:'.$cls;
				echo '<td>'.$subnm.' Marks<br><input type="text" size=3 value="'.$omarks.'" name="mrks[]">';
