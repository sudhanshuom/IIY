...
case 1:
$basic="";
	$sql="Select * from stu_sms where SID=$adminid order by ID DESC";
...
for($xx=2;$xx<4;$xx++)
{
	$basic.='<h3 align="center">Term '.($xx-1).'<h3>';
		$basic.= '<table border="0"  style="'.$clss.' width:100%; font-family:calibri">';
		$basic.= '<tr><th><div align="left">Subject</div><th>M.M<th>Obt.M.';

	$sql1 = "Select * from stu_dmarks WHERE SID=$stuid AND EXAMTP=$xx";
 
		foreach ($dbh->query($sql1) as $row1)
			{
			$sub=findnm("stu_sub","SUB"," WHERE ID=".$row1['SUBID'],$dbh);

		$basic.='<tr><td><div style=" color:black; font-size:0.8em;margin-bottom:6px"><b>'.$sub.'</b></div>';
		$basic.='<td width=15% align="right">'.$row1['TOTM'];
		$basic.='<td width=15% align="right">'.$row1['OBM'];
...
if($pid=="") $basic = "Nothing in Daily Marks";
...
case "8":
$basic="";
	$sql="Select * from stu_calendar order by DATE ASC";
    	foreach ($dbh->query($sql) as $row)
        	{
			$pid =  $row['ID'];			
		
		$basic.= '<table border="0"  style="'.$clss.' width:100%; font-family:calibri">';
		$basic.='<tr><td><div align="left">'.date("d-m-Y",strtotime($row['DATE'])).' &nbsp;  &nbsp;  &nbsp;  &nbsp; '.$row['HOLY'].'</div>';			

		$basic.='<br><div style=" color:black; font-size:0.8em;margin-bottom:6px"><b>'.$row['NM'].'</b></div>';
		$basic.='</center></table>';			
...
case "9":
$basic="";
	$basic.='<table width="100%" border="0" class="table">';
 
	
$sql = "Select DISTINCT DATE from stu_dmarks WHERE SID='$stuid'  AND EXAMTP=0  order by DATE DESC";
  		foreach ($dbh->query($sql) as $row)
			{
		$basic.= '<table border="0"  style="'.$clss.' width:100%; font-family:calibri">';
		$basic.='<tr><td colspan=3 align=center>'.date("d-m-Y",strtotime($row['DATE']));			
			$basic.= '<tr><th><div align="left">Subject</div><th>M.M<th>Obt.M.';

	$sql1 = "Select * from stu_dmarks WHERE DATE='".$row['DATE']."' AND SID=$stuid";
  		foreach ($dbh->query($sql1) as $row1)
			{
			$sub=findnm("stu_sub","SUB"," WHERE ID=".$row1['SUBID'],$dbh);
...
case "10":
$basic="";
	$sql="Select * from stu_driver WHERE TROOT=$adminvan order by ID ASC";
    	foreach ($dbh->query($sql) as $row)
        	{
			$pid =  $row['ID'];			
		
		$basic.= '<table border="0"  style="'.$clss.' width:100%; font-family:calibri">';
		$basic.='<tr><td><div align="left">'.$row['NM'].'</div>';			

		$basic.='Bus Number:'.$row['BUSNO'];
		$basic.='<br>Phone Number:<a href="tel:'.$row['PH'].'">'.$row['PH'].'</a>';
		$basic.='</center></table>';			

			}
		
if($pid=="") $basic = "No van route set";
